...
class Slideshow extends Component {
  ...
  onToggle = event => {
    if (event.target.id === 'heading') {
      this.setState(prevState => ({
        isHeadingEdited: !prevState.isHeadingEdited,
        editParagraph: false,
      }))
    } else {
      this.setState(prevState => ({
  ...
  render() {
    const {isHeadingEdited, editParagraph, activeId, ListOfSlides} = this.state
              ...
              {ListOfSlides.map((slide, index) => (
                ...
                >
                  ...
                  <div className="left-card-container">
                    <h1 className="left-card-heading">{slide.heading}</h1>
                    <p className="left-card-description">{slide.description}</p>
                  </div>
                </li>
              ))}
            </ol>
